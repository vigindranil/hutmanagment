...
const Reports: React.FC<ReportsProps> = ({ onBack }) => {
  ...
  const fetchReportData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_API_URL}user/getReportingDetailsForCompletedHearing`,
        {
          method: "POST",
          headers: myHeaders,
  ...
  const handleExportPDF = () => {
    ...
    // Add title
    doc.setFontSize(18);
    doc.text("Completed Hearing Report", 14, 20);
    ...
    const columns = [
      ...
      { header: "Hearing Date", dataKey: "hearing_date" },
    ];
    ...
    doc.save("Completed_Hearing_Report.pdf");
  };
