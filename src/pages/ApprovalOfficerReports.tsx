...
import Cookies from "js-cookie"; // ✅ for reading token
...
const Reports: React.FC<ReportsProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    //     (item.haat_name && item.haat_name.toLowerCase().includes(term)) ||
    //     (item.shop_owner_name && item.shop_owner_name.toLowerCase().includes(term)) ||
    //     (item.haat_id && item.haat_id.toString().toLowerCase().includes(term))
    //   );
    // }
